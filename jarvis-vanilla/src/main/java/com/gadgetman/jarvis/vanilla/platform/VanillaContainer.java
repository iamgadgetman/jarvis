...
public final class VanillaContainer implements Container {

    private final MinecraftServer server;
    private final net.minecraft.world.Container inv;

    public VanillaContainer(MinecraftServer server, net.minecraft.world.Container inv) {
        this.server = server;
        this.inv = inv;
    }
    ...
    public List<Item> contents() {
        ...
        for (int i = 0; i < inv.getContainerSize(); i++) out.add(VanillaItems.toItem(inv.getItem(i)));
        return out;
    }
    ...
    public List<Item> add(List<Item> items) {
        ...
        for (Item item : items) {
            ...
            addStack(stack);
            if (!stack.isEmpty()) rest.add(VanillaItems.toItem(stack));
        }
        inv.setChanged();
        return rest;
    }
