...
public final class VanillaWorld implements World {

    private final ServerLevel level;

    public VanillaWorld(ServerLevel level) {
        this.level = level;
    }
    ...
    private net.minecraft.world.level.block.state.BlockState at(BlockPos p) {
        return level.getBlockState(VanillaWorlds.mc(p));
    }

    @Override public WorldId id() { return VanillaWorlds.id(level); }
    @Override public String name() { return level.dimension().identifier().getPath(); }
    @Override public Environment environment() { return VanillaWorlds.env(level); }
    ...
    @Override public BlockState block(BlockPos pos) { return VanillaWorlds.state(at(pos)); }

    @Override
    public void setBlock(BlockPos pos, BlockState state, boolean physics) {
        net.minecraft.world.level.block.state.BlockState s = VanillaWorlds.data(server(), state);
        if (s == null) return;
        level.setBlock(VanillaWorlds.mc(pos), s, physics ? Block.UPDATE_ALL : Block.UPDATE_CLIENTS | Block.UPDATE_KNOWN_SHAPE);
    }

    @Override public boolean isSolid(BlockPos pos) { return !at(pos).getCollisionShape(level, VanillaWorlds.mc(pos)).isEmpty(); }
    @Override public boolean isPassable(BlockPos pos) { return at(pos).getCollisionShape(level, VanillaWorlds.mc(pos)).isEmpty(); }
    @Override public boolean isLiquid(BlockPos pos) { return !level.getFluidState(VanillaWorlds.mc(pos)).isEmpty(); }
    ...
    @Override public int blockLight(BlockPos pos) { return level.getBrightness(LightLayer.BLOCK, VanillaWorlds.mc(pos)); }
    @Override public int skyLight(BlockPos pos) { return level.getBrightness(LightLayer.SKY, VanillaWorlds.mc(pos)); }

    @Override
    public String biome(BlockPos pos) {
        String name = level.getBiome(VanillaWorlds.mc(pos)).getRegisteredName();
    ...
    public void breakNaturally(BlockPos pos, Item tool) {
        net.minecraft.core.BlockPos p = VanillaWorlds.mc(pos);
        net.minecraft.world.level.block.state.BlockState s = level.getBlockState(p);
        if (s.isAir()) return;
        Block.dropResources(s, level, p, level.getBlockEntity(p), null, VanillaItems.toStack(server(), tool));
        level.setBlock(p, Blocks.AIR.defaultBlockState(), Block.UPDATE_ALL);
    }

    @Override
    public Entity dropItem(Vec3 at, Item item) {
        ItemEntity drop = new ItemEntity(level, at.x(), at.y(), at.z(), VanillaItems.toStack(server(), item));
        drop.setDefaultPickUpDelay();
        level.addFreshEntity(drop);
        return new VanillaEntity(drop);
    }

    @Override
    public Optional<Container> container(BlockPos pos) {
        return level.getBlockEntity(VanillaWorlds.mc(pos)) instanceof net.minecraft.world.Container c
                ? Optional.of(new VanillaContainer(server(), c))
                : Optional.empty();
    }
    ...
    public List<Entity> nearby(Vec3 center, double radius) {
        ...
        net.minecraft.world.phys.Vec3 c = VanillaWorlds.mc(center);
        ...
        for (net.minecraft.world.entity.Entity e : level.getEntities((net.minecraft.world.entity.Entity) null, box(center, radius, radius, radius))) {
            if (e.position().distanceToSqr(c) <= r2) out.add(new VanillaEntity(e));
        }
        return out;
    }
    ...
    public Optional<Entity> entity(UUID id) {
        ...
        return e == null ? Optional.empty() : Optional.of(new VanillaEntity(e));
    }
    ...
    public BlockScan snapshot(BlockPos min, BlockPos max) {
        ...
        VanillaBlockScan scan = new VanillaBlockScan(from, to, states, loaded);
        for (int cx = from.x() >> 4; cx <= to.x() >> 4; cx++) {
            for (int cz = from.z() >> 4; cz <= to.z() >> 4; cz++) {
                if (!level.hasChunk(cx, cz)) continue;
                LevelChunk chunk = level.getChunk(cx, cz);
                loaded.add(VanillaBlockScan.key(cx, cz));
    ...
    public boolean equals(Object o) {
        return o instanceof VanillaWorld other && other.level.dimension().equals(level.dimension());
    }
