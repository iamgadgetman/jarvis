...
Then talk in chat (for example: `jarvis start mining`). If `natural-language.require-prefix` is `true`, start with `natural-language.prefix` (for example `jarvis ...`); see [Configuration Reference](docs/configuration.md) and [Command Reference](docs/commands.md).
